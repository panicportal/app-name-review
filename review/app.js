...
function mergeCurationStates(local, remote) {
  ...
  Object.entries(remote.records || {}).forEach(([id, incoming]) => {
    ...
    const result = {
      ...
      manual_name_edit_history:
        timestamp(incoming.updated_at) >= timestamp(current.updated_at)
          ? (incoming.manual_name_edit_history || current.manual_name_edit_history || []).slice(-20)
          : (current.manual_name_edit_history || []).slice(-20),
      parts: { ...(current.parts || {}) }
    };
...
async function pushCloudState() {
  if (state.cloudPushPromise) return state.cloudPushPromise;
  state.cloudPushPromise = performCloudPush();
  try { return await state.cloudPushPromise; }
  finally { state.cloudPushPromise = null; if (state.cloudDirty && state.cloudAuthenticated) scheduleCloudSave(); }
}
...
async function performCloudPush() {
  ...
  try {
    ...
    els.saveState.textContent = state.cloudDirty ? "Saving newer changes…" : `Synced · revision ${state.cloudRevision}`;
    setCloudStatus(state.cloudDirty ? "saving" : "online", state.cloudDirty ? "Saving" : "Synced");
  } catch (_) {
    setCloudStatus("offline", "Offline");
...
function scheduleCloudSave() {
  if (!state.cloudAuthenticated) return;
  clearTimeout(state.cloudTimer);
  state.cloudTimer = setTimeout(pushCloudState, state.cloudMode === "offline" ? 5000 : 650);
}
...
function isArtistCustomJapanesePart(part) {
  return Boolean(
    part?.replacement_language === "japanese" && (
      part.replacement_origin_kind === "artist_custom" ||
      /artist[- ](?:created|entered) custom Japanese/i.test([
...
async function requestSurnameDetection({ quiet = false, onlyIfRepair = false } = {}) {
  ...
  let expectedOrigin = requestedOrigin;
  const requestId = state.surnameDetectionRequest = (state.surnameDetectionRequest || 0) + 1;
  const characterId = String(state.selected.id);
  const stillCurrent = () => requestId === state.surnameDetectionRequest &&
    characterId === String(state.selected?.id) &&
    els.fullNameEditSurnameInput.value.trim().toLowerCase() === parsed.surname.toLowerCase() &&
    els.fullNameEditSurnameOrigin.value === expectedOrigin && els.fullNameEditDialog.open;
  ...
  if (state.cloudAuthenticated && requestedOrigin === "auto") {
    const origin = await detectManualNameOrigin("surname_atomic", parsed.surname);
    if (!stillCurrent()) return;
    const japaneseMatch = originMatch(origin, "japanese");
    ...
    if (origin?.origin === "western" && requestedOrigin === "auto") {
      setSurnameEditorOriginMode("western");
      expectedOrigin = "western";
    }
  }
...
function updateFullNameEditPreview() {
  ...
  const surnameChanged = Boolean(parsed.surname && parsed.surname.toLowerCase() !== currentSurname.toLowerCase());
  const lockedSurnameChange = surnameLocked && surnameChanged;
  const lockedSurnameConfirmed = Boolean(els.fullNameEditConfirmLockedSurname.checked);
  els.fullNameEditLockedSurnamePanel.hidden = !lockedSurnameChange;
  const originChanged =
    els.fullNameEditFirstOrigin.value !== els.fullNameEditForm.dataset.originalFirstOrigin ||
    els.fullNameEditSurnameOrigin.value !== els.fullNameEditForm.dataset.originalSurnameOrigin;
  const error = parsed.error ||
    (parsed.first.toLowerCase() !== currentFirst.toLowerCase() && manualFirstNameUsage(parsed.first)
      ...
      : firstLocked && parsed.first !== currentFirst
        ...
        : lockedSurnameChange && !lockedSurnameConfirmed
          ? "This surname is greenlit. Confirm the replacement below to save the new surname."
  ...
  els.fullNameEditStatus.textContent = error || (parsed.japanese
    ? `Japanese surname: one word.${parsed.japanese_source ? ` Connected to ${parsed.japanese_source}.` : " Stored as artist-created without a trait connection."} Save keeps this spelling and the selected bank.`
    : originChanged && parsed.surname === currentSurname && parsed.first === currentFirst
      ? "Only the bank classification will change; your name and decisions stay the same."
      :
...
function openFullNameEditor(editorOptions = {}) {
  ...
  const firstOriginMode = storedOriginMode(character, "first", character.first_name_language || "western");
  const surnameOriginMode = storedOriginMode(character, "surname_part_1", character.surname_language || "western");
  ...
  els.fullNameEditJapaneseSource.innerHTML = `<option value="">Artist-created · no trait connection</option>` + sources.map(source => `<option value="${escapeHtml(source)}">${escapeHtml(source)}</option>`).join("");
  ...
  setFirstEditorOriginMode(firstOriginMode);
  setSurnameEditorOriginMode(surnameOriginMode);
  els.fullNameEditForm.dataset.originalFirstOrigin = firstOriginMode;
  els.fullNameEditForm.dataset.originalSurnameOrigin = surnameOriginMode;
  ...
  const storedJapaneseRoute = partReview(character.id, "surname_part_1").replacement_trait_source || "";
  els.fullNameEditJapaneseSource.value = sources.includes(storedJapaneseRoute) ? storedJapaneseRoute : "";
...
async function saveFullNameEdit(event) {
  ...
  const selectedFirstMode = els.fullNameEditFirstOrigin.value;
  const selectedSurnameMode = els.fullNameEditSurnameOrigin.value;
  ...
  const failOrigin = message => {
    els.fullNameEditStatus.textContent = message;
  ...
  const resolveOriginMode = (mode, detection, label, changed) => {
    ...
    if (detection?.origin === "japanese") return { mode: "japanese_bank", match: originMatch(detection, "japanese") };
    if (detection?.origin === "western") return { mode: "western", match: originMatch(detection, "western") };
    return { error: `${label} could not be classified automatically. Choose Western or Japanese in the bank selector above.` };
  };
  const resolvedFirst = resolveOriginMode(selectedFirstMode, firstOrigin, "First name", firstChanged || firstOriginChanged);
  ...
  if (
    ...
    !els.fullNameEditConfirmLockedSurname.checked
  ) {
    return failOrigin("This surname is greenlit. Confirm the replacement in the editor before converting it to one atomic Japanese surname.");
  }
  ...
  if (parsed.first !== currentFirst || firstOriginChanged) {
    ...
    const detectedFirstMatch = resolvedFirst.match;
    const customFirstRoute = `Artist custom Japanese first name · Clothing:${character.clothing || "No clothing trait"} + Body:${character.gender_from_body}`;
    const detectedFirstRoute = resolvedFirst.mode === "japanese_custom"
      ? customFirstRoute
      : detectedFirstMatch?.route || `Clothing:${character.clothing || "No clothing trait"}`;
    record.parts.first = {
      ...current,
      decision: parsed.first !== currentFirst ? "replace" : current.decision,
      scope: parsed.first !== currentFirst ? "this_character" : current.scope,
      disabled: false,
      replacement_value: parsed.first,
      replacement_source: resolvedFirst.mode === "japanese_custom"
        ? "Artist-entered custom Japanese first name · explicit origin"
        : detectedFirstOrigin === "japanese"
      ...
      replacement_origin_confirmed_by_user: selectedFirstMode !== "auto",
      replacement_origin_kind: resolvedFirst.mode === "japanese_custom"
        ? "artist_custom"
        : resolvedFirst.mode === "japanese_bank" ? "authoritative_bank" : "curated_or_manual_western",
      replacement_rationale: resolvedFirst.mode === "japanese_custom"
        ? `Artist explicitly classified “${parsed.first}” as a custom Japanese first name. It is not represented as an exact closed-CSV match. Gender routing remains Body:${character.gender_from_body}.`
        : detectedFirstOrigin === "japanese"
  ...
  if (parsed.surname !== currentSurname || surnameOriginChanged || (needsSurnameComponentRepair(character) && !parsed.preserve_unresolved_surname)) {
    const current1 = partReview(character.id, "surname_part_1");
    const current2 = partReview(character.id, "surname_part_2");
    const replacingLockedSurname = surnameChanged &&
      [current1, current2].some(part => part.decision === "approve") &&
      els.fullNameEditConfirmLockedSurname.checked;
    if (parsed.japanese) {
      const japaneseMatch = resolvedSurname.match;
      const japaneseCustom = resolvedSurname.mode === "japanese_custom";
      const selectedJapaneseRoute = parsed.japanese_source || "";
      const japaneseRoute = selectedJapaneseRoute || (japaneseCustom
      ...
      const routeNote = selectedJapaneseRoute ? ` Artist connected it to ${selectedJapaneseRoute}.` : "";
      record.parts.surname_part_1 = { ...current1, decision: replacingLockedSurname ? "approve" : parsed.surname !== currentSurname ? (current1.decision || "replace") : current1.decision, scope: parsed.surname !== currentSurname ? (current1.scope || "this_character") : current1.scope, disabled: false, replacement_value: parsed.surname, replacement_source: japaneseCustom ? "Artist-entered custom Japanese surname · explicit origin" : "Japanese names surnames.csv · exact authoritative bank match", replacement_trait_source: japaneseRoute, replacement_language: "japanese", replacement_origin_confirmed_by_user: selectedSurnameMode !== "auto", replacement_origin_kind: japaneseCustom ? "artist_custom" : "authoritative_bank", replacement_rationale: japaneseCustom ? `Artist explicitly classified “${parsed.surname}” as a custom Japanese surname. It is stored as one atomic surname and is not represented as an exact closed-CSV match.${routeNote}` : `Exact artist Japanese surname entry for ${japaneseRoute}. Detected automatically during the full-name edit for Surv!vor #${character.id}.`, replacement_scores: null, updated_at: timestamp, reviewer: state.curation.reviewer || current1.reviewer || "", deleted_at: null };
      record.parts.surname_part_2 = { ...current2, decision: replacingLockedSurname ? null : current2.decision, scope: replacingLockedSurname ? null : current2.scope, disabled: true, updated_at: timestamp, reviewer: state.curation.reviewer || current2.reviewer || "", deleted_at: null };
    } else {
      ...
      parsed.components.forEach((component, index) => {
        ...
        record.parts[key] = {
          ...current,
          decision: replacingLockedSurname ? "approve" : parsed.surname === currentSurname ? current.decision : (current.decision || "replace"),
          scope: parsed.surname === currentSurname ? current.scope : (current.scope || "this_character"),
          disabled: false,
          ...
          replacement_origin_confirmed_by_user: selectedSurnameMode !== "auto",
          replacement_origin_kind: "manual_western_trait_component",
    ...
    record.normalized_name = {
      ...
      surname_format_version: SURNAME_FORMAT_VERSION,
      derivation_method: parsed.japanese
        ? resolvedSurname.mode === "japanese_custom" ? "manual_atomic_japanese_custom" : "manual_atomic_japanese_bank"
        : "manual_structured",
      needs_surname_component_repair: false
    };
    record.normalized_name_updated_at = timestamp;
    if (replacingLockedSurname) {
      record.manual_name_edit_history = [...(record.manual_name_edit_history || []), {
        changed_at: timestamp,
        reviewer: state.curation.reviewer || "",
        action: "replace_confirmed_surname",
        previous_surname: currentSurname,
        resulting_surname: parsed.surname,
        resulting_language: parsed.japanese ? "japanese" : "western",
        selected_trait_source: parsed.japanese_source || null
      }];
    }
...
function exportRecord(character) {
  ...
  partDefinitions(character).forEach(part => {
    ...
    parts[part.key] = {
      ...
      replacement_origin_kind: review.replacement_origin_kind || "",
  ...
  payload.records.forEach(importedRecord => {
    ...
    PART_KEYS.forEach(key => {
      ...
      local.parts[key] = {
        ...
        replacement_origin_kind: incoming.replacement_origin_kind || "",
...
function handoffFirstNameBankText(character, firstContext = {}) {
  ...
  return [
    ...
    `If an exact ${character.clothing} + ${character.gender_from_body} Markdown bank is attached in this ChatGPT project/conversation, open it and treat its exact rows as authoritative even when Name Studio reports no uploaded MD file. Never replace an available matching MD bank with fallback names.`,
...
function compactFirstNameBankText(character, firstContext = {}) {
  ...
  const fileSlug = `${character.clothing}-${character.gender_from_body}`
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
function compactChatGptHandoffText(character, context = {}) {
  ...
  const uploadedFirstRows = inlineFirstRows.filter(candidate => candidate.origin === "Uploaded MD bank");
  const uploadedFirstFiles = firstContext.matchingFiles || [];
  const fallbackOrigins = [...new Set(inlineFirstRows.map(candidate => candidate.origin).filter(Boolean))];
  const firstSourceProof = uploadedFirstRows.length && uploadedFirstFiles.length
    ? `Begin the first-name section exactly with: “FIRST-NAME MD FILE: ${uploadedFirstFiles.join(", ")}”. Copy the filename exactly so the artist can verify which file was used.`
    : `Begin the first-name section exactly with: “FIRST-NAME SOURCE: ${fallbackOrigins.join(" + ") || "no verified bank"} — no uploaded MD file embedded”. Never claim an MD filename when Name Studio did not embed one.`;
  const surnameOpen = ["surname_part_1", "surname_part_2"].some(key =>
    partDefinitions(character).find(part => part.key === key)?.available &&
    partReview(character.id, key).decision !== "approve"
  );
  ...
  return [
    ...
    `- Maximum final response: 1,300 words. Use compact tables and no prose essays. Do not restate this packet or the master rules.`,
...
async function copyCompactReviewPacket() {
  ...
  try {
    ...
    const inlineRows = Math.min(30, compact.firstContext.candidates.length);
    await copyText(
      compact.packet,
      mdRows
        ? `Copied compact rotation ${compact.rotationPass}: ${Math.min(30, mdRows)} verified MD rows embedded directly and 48 diverse surnames requested.`
        : `Copied compact rotation ${compact.rotationPass}: ${inlineRows} verified local-bank names embedded and 48 diverse surnames requested.`
    );
  } catch (error) {
...
function bindEvents() {
  ...
  els.reviewSourceIssues.addEventListener("click", () => {
    els.statusFilter.value = "surname-repair";
    setViewMode("browse");
    setMobileRoster(true);
    applyFilters();
  });
  els.exportSourceAudit.addEventListener("click", async () => {
    els.exportSourceAudit.disabled = true;
    try {
      const response = await fetch("/api/surname-repair?all=1", { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw Error(payload.error || "Sign in to download the shared audit.");
      downloadExportFile(new Blob([JSON.stringify(payload, null, 2)], {type:"application/json"}), `surname-source-audit-r${payload.revision}.json`);
      showToast(`Downloaded ${payload.detected} source issues, grouped by Clothing.`, "success");
    } catch (error) { showToast(error.message, "warning"); }
    finally { els.exportSourceAudit.disabled = false; }
  });
